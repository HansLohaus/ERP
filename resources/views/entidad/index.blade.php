  ...
  <div class="panel panel-default">
    <div class="panel-body">
      ...
      <div class="pull-right">
        <div class="btn-group">
          <a href="{{ route('entidades.create') }}" class="btn btn-info" >Añadir Entidad</a>
        </div>
      </div>
        ...
        <table id="tabla-logs" class="table table-bordred table-striped">
          ...
          @foreach($entidades as $entidad)  
            <tr>
              ...
              <td>{{$entidad->nombre_fantasia}}</td>
              <td><a data-entidad="{{json_encode($entidad)}}" href="#" class="btn btn-info" data-toggle="modal" data-target="#mas_info">más info</a></td>
              <td><a class="btn btn-primary" href="{{action('EntidadController@edit', $entidad->id)}}" ><i class="bi bi-pencil"></i></a></td>
              <td><form action="{{action('EntidadController@destroy', $entidad->id)}}" method="post">
              {{csrf_field()}}
              <input name="_method" type="hidden" value="DELETE"><button class="btn btn-danger" type="submit" onclick="return confirm('Se eliminaran todos los servicios y facturas asociados a la entidad, ¿Seguro que quieres eliminar? ')"><i class="bi bi-trash"></i></button>
              </td>
              </tr>
              @endforeach 
              @else
...
<script>
  var datatable_tabla = $("#tabla-logs").dataTable({
    ...
    "iDisplayLength": 30,
    "language": {
        "emptyTable": "No hay entidades en el sistema",
        "paginate": {
            "next":       "Siguiente",
            "previous":   "Anterior"
        },
        "search": "Filtrar por :",
        "zeroRecords": "No se encuentran entidades para la busqueda solicitada"
    },
